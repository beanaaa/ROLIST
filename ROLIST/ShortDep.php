...
if ((isset($_POST['hd_del'])) && ($_POST['hd_del'] != "")) {
  ...
  $deleteSQL = sprintf("delete from DepTemp where idx = %s and Hospital_ID = %s",
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
 	...
 	$sql_test = mysql_query("select * from DepTemp where Hospital_ID = $h_id");
 	$sql_test_rows = mysql_num_rows($sql_test); // mysql row 수를 판단하여 중복 유/무 판별
  
 	if($sql_test_rows>'0'){
 	$sql_idx = mysql_query("select idx from DepTemp where Hospital_ID = $h_id");   
 	...
 	$updateSQL = sprintf("insert into DepTemp(Hospital_ID, Memo1, Date1, idx) values (%s, %s, %s, $j)" ,
    ...
    $updateSQL = sprintf("update DepTemp SET Memo1 = %s, Date1=%s where Hospital_ID = $h_id and idx = '$idx_i'",
  ...
  if($sql_test_rows=='0'){
	...
	$updateSQL = sprintf("insert into DepTemp(Hospital_ID, Memo1, Date1, idx) values (%s, %s, %s, 1)" ,
                       GetSQLValueString($_POST['txt_hospital_id'] ,"text"),
                       GetSQLValueString($_POST['txt_Memo'], "text"),
                       GetSQLValueString($_POST['txt_Date'], "text")
                       //GetSQLValueString($colname_patientinfo, "int"),
...
$query_TempMemo = sprintf("SELECT * FROM DepTemp WHERE Hospital_ID = %s", GetSQLValueString($colname_patientinfo, "text"));
...
/*
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("INSERT INTO DepTemp (Hospital_ID, Date, Memo) VALUES(%s, %s, %s)",
...
<tr>
 ...
 <td width="40%">
<form id = "form11" name = "form11" method="POST" action = "ShortDep.php">
...
<?php /* $updateSQL = sprintf("INSERT INTO DepTemp (Hospital_ID,Memo1, Date1) VALUES(%s, %s, '2015-12-08')",                                             
                       GetSQLValueString($colname_patientinfo, "int"), GetSQLValueString($_POST['txt_Memo'], "text"));
                       echo $updateSQL; ?>*/

/*
<?php $temp = sprintf("INSERT INTO DepTemp (Hospital_ID,Memo1, Date1) VALUES(%s, %s, '2015-12-08')",                                             
  ...
  <table width="70%" border="0" cellspacing="1" cellpadding="1" align ="center">
	...
	//echo $Today_Date;    
	$sql_Memo = mysql_query("select Memo1 from DepTemp where Hospital_ID = '$h_id'");
	$sql_Date = mysql_query("select Date1 from DepTemp where Hospital_ID = '$h_id'"); 
	$sql_idx = mysql_query("select idx from DepTemp where Hospital_ID = '$h_id'");  
   ...
   ?>
      <tr>
<!-- 	      <th scope="row" width="70px" style="color: #1C73B9"><?php echo $i+1;?>. Note : </th> -->
		  <td rows="2" cols="50"><input = "text" id = "<?php echo $txt_comment?>" name = "<?php echo $txt_comment ?>" value = "<?php echo $Memo; ?>"/></td>
<!-- 		  <th width="50">Date : </th>  -->
		  <td><input = "text" id = "<?php echo $txt_comment_date?>" name = "<?php echo $txt_comment_date?>" value="<?php echo $Date;?>" size = "5"/> <td>
		  <form id="form2" name="form2" method="POST" action="ShortDep.php">
...
<p>Date: </p>
    ...
    <tr><th width="70px" scope="row" style="color: #1C73B9">Add Note : </th>
      <td><textarea name="txt_Memo" id = "txt_Memo" type = "text" rows = "2" cols = "70" ></textarea>
         <th width="20" style="color: #1C73B9">Date : </th> 
         <td width="100">
<!-- 	         <input name="txt_Date" type="text" id="txt_Date" value=<?php echo $Today_Date; ?> size = "3" /> -->
